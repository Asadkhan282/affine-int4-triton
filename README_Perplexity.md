...
more accurately than NF4 — 316 of 322 layers across two model families — and produces
lower wikitext-2 perplexity on all three models tested, while running at latency parity
with NF4 and 2.0–2.1x faster than cuBLAS fp16.

The perplexity advantage scales with how much 4-bit hurts the model: −0.37 on
Qwen2.5-0.5B (which degrades 14%), −0.15 on Qwen2.5-1.5B (10%), and −0.03 on
TinyLlama-1.1B (4%) — the last being within noise. Where quantization is nearly free,
format choice is nearly irrelevant.
...
| `sweep_layers.py` | Weight-reconstruction accuracy across every projection of a model, affine int4 vs NF4 at matched block size. |
| `perplexity_eval.py` | End-to-end wikitext-2 perplexity for fp16, affine int4 and NF4 under identical treatment. |
...
Latency measured on a shared T4 and would benefit from a dedicated machine.
Perplexity is one dataset at one sequence length. Both formats use naive
round-to-nearest with no calibration, so the absolute degradation figures are worse
than production methods like GPTQ or AWQ achieve — this compares formats, not
