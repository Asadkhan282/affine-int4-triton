...
A proper test would vary bit width or group size within one model to move sensitivity
while holding architecture fixed. TODO.
...
- [x] End-to-end perplexity — done, three models, affine int4 ahead on all
- [ ] Byte-for-byte metadata accounting for both formats at block 32
- [ ] Vary bit width within one model to separate sensitivity from architecture
- [ ] Compare against a calibrated method (GPTQ/AWQ) to place these numbers in context
