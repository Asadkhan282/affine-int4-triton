...
on 316 of 322 layers, median ratio ≈0.94x.
...
| bitsandbytes | 0.50.2 |
| driver | 580.159.04 |
| Weights tested | Qwen2.5-0.5B (168 weights) and TinyLlama-1.1B (154 weights), kurtosis 3.4–300; `randn` synthetic for kernel checks |
...
This predicts the advantage should continue growing below 32 — untestable against NF4,
but testable on the affine side alone. TODO.
...
Both formats at 32 carry comparable overhead: affine int4 stores a scale and a
zero-point per group (25% of packed weight bytes at fp16), NF4 stores an absmax per
block plus its own nested quantization state. Exact byte-for-byte accounting: TODO —
this matters for the fairness of the comparison and should be measured, not assumed.
...
No end-to-end model evaluation was run. RMSE on weights is a proxy for output quality,
not a substitute for it — a 5% reconstruction improvement may or may not move
perplexity, and the two are not linearly related. This is the largest gap in the work.
...
- [x] Sweep all layers, attention and MLP — done, 168 weights, 164 wins
- [x] Match block sizes between the two schemes — done, 0.947x at 32
- [ ] Re-run everything in one clean session so every number comes from one run
- [x] Version strings — torch 2.10.0+cu128, triton 3.6.0, bitsandbytes 0.50.2, driver 580.159.04
- [ ] Byte-for-byte metadata accounting for both formats at block 32
- [x] Second model family — done, TinyLlama-1.1B, 152/154 at 0.932x
